  ...
  return (
    <Fragment>
      <Box display="flex" justifyContent="space-between">
        ...
        <Typography variant="h5" ref={refGrade}>
          Score:{" "}
        </Typography>
      </Box>
      <Box>
        {quiz.map((quiz, id) => (
          <Card key={id} className={classes.root}>
            <CardContent>
              <Typography component="p">{quiz.question}</Typography>
            ...
            <div className={classes.boxCorrect}>
              {boxCorrectAnswer[id] === false && btnSubmitHasClick ? (
                <Typography>{quiz.answer[quiz.correctAnswer]}</Typography>
              ) : (
                <></>
              )}
            </div>
        ...
        {lengthQuestion > 0 ? (
          <Box display="flex" flexDirection="column" marginTop={4} marginBottom={4}>
            <Button disabled={disableButton} variant="contained" color="primary" onClick={checkQuestionThatAnswered}>
              Submit
            </Button>
...
function MultipleChoice({ answer }) {
  ...
  return (
    <>
      {result?.map((radio, id) => (
        <FormControlLabel className={classes.root} key={id} label={radio.answer} value={radio.choice} control={<Radio />} />
      ))}
    </>
  );
}
